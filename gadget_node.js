...
var make_evaluator = function(article_title, article_id, revision_id, dom, rewards) {
    ...
    self.add_input('editorStats', basic_query('http://en.wikipedia.org/w/api.php?action=query&prop=revisions&titles=' + article_title + '&rvprop=user&rvlimit=50&format=json'), editorStats);
    self.add_input('inLinkStats', basic_query('http://en.wikipedia.org/w/api.php?action=query&format=json&list=backlinks&bltitle=' + article_title + '&bllimit=500&blnamespace=0'), inLinkStats);
    self.add_input('feedbackStats', basic_query('http://en.wikipedia.org/w/api.php?action=query&list=articlefeedback&afpageid=' + article_id + '&afuserrating=1&format=json&afanontoken=01234567890123456789012345678912'), feedbackStats);
    self.add_input('searchStats', basic_query('http://ajax.googleapis.com/ajax/services/search/web?v=1.0&q=' + article_title), searchStats);
    self.add_input('newsStats', basic_query('http://ajax.googleapis.com/ajax/services/search/news?v=1.0&q=' + article_title), newsStats);
    self.add_input('wikitrustStats', yql_query('select * from html where url ="http://en.collaborativetrust.com/WikiTrust/RemoteAPI?method=quality&revid=' + revision_id + '"', 'json'), wikitrustStats);
    ...
    self.add_input('getAssessment', basic_query('http://en.wikipedia.org/w/api.php?action=query&prop=revisions&titles=Talk:' + article_title + '&rvprop=content&redirects=true&format=json'), getAssessment);
...
function print_page_stats(ev) {
    console.log("Page stats for "+ev.article_title);
    ...
    do_query('http://en.wikipedia.org/w/api.php?action=query&prop=revisions&titles='+article_title+'&rvprop=ids&redirects=true&format=json',
            function(data) {
                ...
                if (page_ids.length > 0) {
                    ...
                    var article_title = pages[article_id].title;
                    var rev_id        = pages[article_id].revisions[0].revid;
                    var prev_rev_id   = pages[article_id].revisions[0].parentid;
...
function get_article(article_title, article_id, revision_id) {
    ...
    do_query('http://en.wikipedia.org/w/api.php?action=parse&page='+article_title+'&redirects=true&format=json',
         function(data) {
            var text = data.parse.text['*'];
         
            var get_article_end = (new Date).getTime(); //time
            var get_article_time = get_article_end - get_article_start;
            ...
            jsdom.env({
                html: text,
                    ...
                    var parse_article_end = (new Date).getTime();
                    var parse_article_time = parse_article_end - parse_article_start;
                    ...
                    var ev = make_evaluator(article_title, article_id, revision_id, window, rewards);
