...
def contains_num(founds):
    ret = []
    for found in founds:
        if search('18[0-9]+', get_text(found)):
...
class DOM(Input):
    ...
    stats = {
        'cat_contains_history': lambda f: len(f('#mw-normal-catlinks a[href*="History"]') + f('#mw-normal-catlinks a[href*="history"]')),
        'cat_contains_military': lambda f: len(f('#mw-normal-catlinks a[href*="Military"]') + f('#mw-normal-catlinks a[href*="military"]')),
        'cat_contains_1800': lambda f: len(contains_num(f('#mw-normal-catlinks a[href*="18"]'))),
        'cat_contains_19th_century': lambda f: len(f('#mw-normal-catlinks a[href*="19th-century"]')),
        'intro_p_contains_1800': lambda f: True if len(contains_num(f('#toc').prevAll('p'))) else False,
        'infobox_contains_1800': lambda f: True if len(contains_num(f('.infobox'))) else False,
        'total_history_freq': lambda f: word_frequency('history', get_text(f('#content')[0])),
